...
# Paths to data
df = pd.read_csv("./encode_time.csv")
...
plot_data = {
    ...
    'Original Ratio': df[df['Encoding'].isin(original_encodings)].set_index('Encoding').loc[original_encodings, 'Compression Ratio'].values,
    'Improved Ratio': df[df['Encoding'].isin(improved_encodings)].set_index('Encoding').loc[improved_encodings, 'Compression Ratio'].values
}
...
axs[0].set_ylabel('Compression Time (ns/point)')
...
# Save the figure
fig.savefig("./fig/encode_time_comparison.eps", format='eps', dpi=400, bbox_inches='tight')
fig.savefig("./fig/encode_time_comparison.png", dpi=400, bbox_inches='tight')
