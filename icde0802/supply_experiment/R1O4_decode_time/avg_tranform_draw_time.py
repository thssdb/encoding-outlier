...
path_prune_ratio = parent_dir + 'bos_m/'

path_test_beta_ratio = ['./compression_ratio/bos_m/','./compression_ratio/bos_b/','./compression_ratio/bos_v/']
path_pfor_ratio = [path_bos_ratio,path_bws_ratio,path_prune_ratio]
...
df_result = pd.DataFrame(columns=['Encoding','Dataset','Decode Time','Ratio'])
result_i = 0
result_count = 0
for j in range(len(dir_r)):
    ...
    for path_test_ratio in path_test_beta_ratio:
        ...
        for i in range(len_test_df):
            df_result.loc[result_count] = [df_avg_test.iloc[i,0]+"-Improve",dir_r[j],df_avg_test.iloc[i,-1],1/df_avg_test.iloc[i,-2]]
            result_count += 1 
    ...
    for path_test_ratio in path_pfor_ratio:
        ...
        for i in range(len_test_df):
            df_result.loc[result_count] = [df_avg_test.iloc[i,0],dir_r[j],df_avg_test.iloc[i,-1],1/df_avg_test.iloc[i,-2]]
            result_count += 1 
